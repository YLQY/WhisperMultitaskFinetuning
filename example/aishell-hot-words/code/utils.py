...
class GenerationMixin:
    ...
    def generate(
        ...
        **kwargs,
        ...
        #print("lainspeech",hot_tree)
        """
        lists = hot_tree.find_path_add_score_return_idx([35,68,97,5,6,6,5363,66,66666])
        if lists is not None:
            for item in lists:
                print(item.value)
                pass
        else:
            for item in hot_tree.child_nodes:
                print(item.value)
                pass
        """
        r"""
        ...
        if generation_mode == GenerationMode.ASSISTED_GENERATION:
            ...
            result = self._beam_search(
                ...
                synced_gpus=synced_gpus,
                hot_tree=hot_tree,
                hot_words_score=hot_words_score,
                **model_kwargs,
            )
    ...
    def _beam_search(
        ...
        **model_kwargs,
        ...
        while self._has_unfinished_sequences(this_peer_finished, synced_gpus, device=input_ids.device):
            ...
            next_token_logits = outputs.logits[:, -1, :].clone()
            #print("------------第",cur_len,"次解码--------------")
            #print("Whisper模型原始输出：",np.shape(next_token_logits))
            ...
            #print("拉直了得分矩阵维度：",np.shape(next_token_scores))

            # Beam token selection: pick 1 + eos_token_id.shape[0] next tokens for each beam so we have at least 1
            # non eos token per beam.
            n_eos_tokens = eos_token_id.shape[0] if eos_token_id is not None else 0
            n_tokens_to_keep = max(2, 1 + n_eos_tokens) * num_beams
            #print("保留的最大token个数",n_tokens_to_keep,n_eos_tokens)
            if do_sample:
                probs = nn.functional.softmax(next_token_scores, dim=-1)
